...
std::unique_ptr<PolygonMesh> OBJSerializer::loadMeshData(const std::string& filename) {
    ...
    return std::make_unique<PolygonMesh>(std::move(finalVertices), std::move(faces), std::move(finalTexCoords), std::move(finalNormals));
}
